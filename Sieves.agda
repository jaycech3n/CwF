...
{-
We describe the shape of partial k-skeleta of n-simplices in which the first t
k-faces are present by "downward sieves" (n,k,t). More precisely: order each
Hom([m],[n]) lexicographically by the image of its elements. Then a downward
sieve (n,k,t) is the sieve into [n] generated by the first t arrows [k] → [n]
together with all the arrows [k-1] → [n]. In this development, we simply call
these "sieves".
-}
...
-- [n, k, t]∩[m, f] calculates the shape of the intersection of the (downward)
-- sieve (n,k,t) with the sieve (not generally downward) generated by a face map
-- f : m →+ n.
