...
{- Remark on base cases in definitions:

Some other constructions in the literature consider *augmented* semi-simplicial
sets/types which start at height h = -1, so that one can e.g. define
  M 0 = M[ 0 , 0 , 0 ],
and also define
  M[ i , h , 1+ t ]
uniformly without having to split into cases for h ∈ {0, 1+ _}.

We avoid this approach since we have to explicitly index CwF contexts, and
coerce types and terms between different context extensions. -}
...
M⃗[ i , O , 1+ t ] iS f hᵢ⁺ hₒ⁺ ⦃ uₒ ⦄ x
 with M⃗[ i , O , t ] (shape-from-next-t iS) f hᵢ⁺ hₒ⁺
...
M⃗[ i , O , 1+ t ] iS f hᵢ⁺ hₒ⁺ ⦃ uₒ ⦄ x
    | _
    | inl yes = {!!}
M⃗[ i , 1+ h , O ] iS {m} f hᵢ⁺ ⦃ uᵢ ⦄ hₒ⁺ ⦃ uₒ ⦄ x =
  M⃗[ i , h , Hom-size i h ] (shape-from-next-h iS) f hᵢ⁺ ⦃ _ ⦄ hₒ⁺ x
M⃗[ i , 1+ h , 1+ t ] iS {m} f hᵢ⁺ ⦃ uᵢ ⦄ (1+ hₒ⁺) ⦃ ltSR uₒ ⦄ x =
  tr Tm
     (M-unc-at-S ([ i , 1+ h , 1+ t ]∩[ m , f ] iS) hₒ⁺ ⦃ uₒ ⦄ ⦃ ltSR uₒ ⦄)
     (M⃗[ i , 1+ h , 1+ t ] iS {m} f hᵢ⁺ hₒ⁺ ⦃ uₒ ⦄ x ⁺ₜ)
M⃗[ i , 1+ h , 1+ t ] iS {m} f hᵢ⁺ ⦃ uᵢ ⦄ (1+ ∩-height) ⦃ ltS ⦄ x = {!!}
